...
function buildWalls(level) {
  ...
  const hLine = (x1, x2, y) => {
    for (let x = x1; x <= x2; x++) add2(x, y);
  };
  const vLine = (y1, y2, x) => {
    for (let y = y1; y <= y2; y++) add2(x, y);
  };
...
class AudioEngine {
  ...
  playTone(freq, type, startTime, duration, gainPeak, attack = 0.04, release = 0.3) {
...
function App() {
  ...
  const speedRef = reactExports.useRef(levelStartSpeed(1));
  ...
  const spawnAllFoods = reactExports.useCallback(() => {
    ...
    for (let x = 1; x < COLS - 1; x++) {
      for (let y = 1; y < ROWS - 1; y++) {
        const key = `${x},${y}`;
        if (!wallsRef.current.has(key) && !snakeSet.has(key) && (x * 3 + y * 7) % Math.round(1 / FOOD_DENSITY) === 0) {
          foods.push({ x, y });
        }
      }
    }
